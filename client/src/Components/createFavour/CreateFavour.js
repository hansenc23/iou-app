...
const CreateFavour = ({ setType }) => {
  ...
  function handleOnChangeWhom(text, fromSelect = false) {
    ...
    if (username && !fromSelect) {
      axios.post(`${process.env.REACT_APP_API_URL}/auth/username_predict`, { username }).then((response) => {
  ...
  async function handleCreateFavor() {
    ...
    if (storedAction === 'Owe') {
      ...
      try {
        const response = await axios.post(`${process.env.REACT_APP_API_URL}/auth/username_predict`, { username });
